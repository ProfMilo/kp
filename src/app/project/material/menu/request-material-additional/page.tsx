...
import { Package, Search, Filter, Download, Upload } from 'lucide-react';
...
interface RequestMaterialAdditionalData {
  id: string;
  year: string;
  region: string;
  shipper: string;
  ...
  steelClam: number;
  stopping: number;
  beritaAcaraFile: File | null;
  beritaAcaraFileName: string;
  beritaAcaraFileUrl?: string; // Store the download URL from Firebase Storage
}

const RequestMaterialAdditionalPage: React.FC = () => {
  const [userData, setUserData] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [filteredData, setFilteredData] = useState<RequestMaterialAdditionalData[]>([]);
  ...
  const [requestMaterialRecords, setRequestMaterialRecords] = useState<RequestMaterialAdditionalData[]>([]);

  const [formData, setFormData] = useState<Omit<RequestMaterialAdditionalData, 'id'>>({
    ...
    beritaAcaraFile: null, beritaAcaraFileName: '', beritaAcaraFileUrl: '',
  });
  ...
  const numericKeys: Array<keyof Omit<RequestMaterialAdditionalData, 'id' | 'beritaAcaraFile' | 'beritaAcaraFileName'>> = [
  ...
  const handleFormChange = (key: keyof Omit<RequestMaterialAdditionalData, 'id'>, value: string) => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setFormData(prev => ({ 
        ...prev, 
        beritaAcaraFile: file,
        beritaAcaraFileName: file.name 
      }));
    }
  };
  ...
  // Filter functions
  const applyFilters = (data: RequestMaterialAdditionalData[]) => {
  ...
  const clearFilters = () => {
    setFilters({
      year: '', region: '', shipper: '', driver: '', noPoi: '',
      siteIdDest: '', siteNameDestination: '', siteType: '',
      pole7m: { min: '', max: '' }, poleHc7m: { min: '', max: '' }, pole9m: { min: '', max: '' },
      foCableAdss24c: { min: '', max: '' }, foCableAdss36c: { min: '', max: '' }, foCableAdss48c: { min: '', max: '' }, foCableAdss144c: { min: '', max: '' }, foCableAdss288c: { min: '', max: '' },
      fdt48: { min: '', max: '' }, fdt72: { min: '', max: '' }, fat: { min: '', max: '' },
      splitter14: { min: '', max: '' }, jc24: { min: '', max: '' }, jc36: { min: '', max: '' }, jc48: { min: '', max: '' }, jc144: { min: '', max: '' },
      cableBlackFts: { min: '', max: '' }, slingWr: { min: '', max: '' }, plateBolt20m: { min: '', max: '' }, suspensionCl: { min: '', max: '' }, buildingO: { min: '', max: '' }, grip: { min: '', max: '' }, poleClampSi: { min: '', max: '' }, endClam: { min: '', max: '' }, steelClam: { min: '', max: '' }, stopping: { min: '', max: '' }
    });
  };
  ...
  const exportToCSV = () => {
    ...
    try {
      ...
      link.setAttribute('download', `request_material_additional_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  useEffect(() => {
    const q = query(collection(db, 'material_request_additional'), orderBy('createdAt', 'desc'));
    const unsub = onSnapshot(q, (snapshot) => {
      const records: RequestMaterialAdditionalData[] = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      })) as RequestMaterialAdditionalData[];
      setRequestMaterialRecords(records);
    });
    return () => unsub();
  }, []);
  ...
  useEffect(() => {
    let filtered = requestMaterialRecords;
  ...
  }, [searchTerm, requestMaterialRecords, filters]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const emptyFields = requiredFields.filter(field => {
      const value = formData[field as keyof Omit<RequestMaterialAdditionalData, 'id'>];
    ...
    if (!formData.beritaAcaraFile) {
      alert('Please upload a Berita Acara file');
      return;
    }
    ...
    try {
      ...
      let fileUrl = '';
      if (formData.beritaAcaraFile) {
        const storageRef = ref(storage, `berita-acara-files/${Date.now()}_${formData.beritaAcaraFileName}`);
        const uploadResult = await uploadBytes(storageRef, formData.beritaAcaraFile);
        fileUrl = await getDownloadURL(uploadResult.ref);
      }
      
      // Save record to Firestore with file URL
      const docRef = await addDoc(collection(db, 'material_request_additional'), {
        ...formData,
        beritaAcaraFile: formData.beritaAcaraFileName, // Keep filename for reference
        beritaAcaraFileUrl: fileUrl, // Store the actual download URL
      ...
      alert('Request Material Additional record saved successfully.');
    } catch (err) {
      console.error('Failed to save request material additional record', err);
      alert('Failed to save request material additional record.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading Request Material Additional Data...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-full mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center mb-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Request Material Additional</h1>
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">Request Material Additional Data</h3>
              <div className="text-sm text-gray-600">
                Showing {filteredData.length} of {requestMaterialRecords.length} records
              </div>
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            {filteredData.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900">No request material additional records found</h3>
                <p className="mt-1 text-sm text-gray-500">Get started by adding a new request material additional record using the form below.</p>
              </div>
            ) : (
                          ...
                          {item.beritaAcaraFileName ? (
                            <button
                              onClick={() => {
                                ...
                                if (item.beritaAcaraFileUrl) {
                                  // Download the file using the stored URL
                                  const link = document.createElement('a');
                                  link.href = item.beritaAcaraFileUrl;
                                  link.download = item.beritaAcaraFileName || 'document.pdf';
                                  link.target = '_blank';
                                  document.body.appendChild(link);
                                  link.click();
                                  document.body.removeChild(link);
        ...
        {/* Add New Request Material Additional Record */}
        {userData?.role?.toLowerCase() === 'rpm' && (
          <div className="bg-white rounded-lg shadow-md p-6 mt-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Add Request Material Additional Record</h3>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  <label htmlFor="rma-year" className="block text-sm text-gray-600 mb-1">Year</label>
                  <input id="rma-year" value={formData.year} onChange={e=>handleFormChange('year', e.target.value)} className="w-full border rounded px-3 py-2 text-black" />
                </div>
                <div>
                  <label htmlFor="rma-region" className="block text-sm text-gray-600 mb-1">Region</label>
                  <div className="relative">
                    <input 
                      id="rma-region" 
                      ...
                      list="rma-region-options"
                    />
                    <datalist id="rma-region-options">
                ...
                <div>
                  <label htmlFor="rma-shipper" className="block text-sm text-gray-600 mb-1">Shipper</label>
                  <input id="rma-shipper" value={formData.shipper} onChange={e=>handleFormChange('shipper', e.target.value)} className="w-full border rounded px-3 py-2 text-black" />
                </div>
                <div>
                  <label htmlFor="rma-driver" className="block text-sm text-gray-600 mb-1">Driver</label>
                  <input id="rma-driver" value={formData.driver} onChange={e=>handleFormChange('driver', e.target.value)} className="w-full border rounded px-3 py-2 text-black" />
                </div>
                <div>
                  <label htmlFor="rma-nopoi" className="block text-sm text-gray-600 mb-1">No POI</label>
                  <input id="rma-nopoi" value={formData.noPoi} onChange={e=>handleFormChange('noPoi', e.target.value)} className="w-full border rounded px-3 py-2 text-black" />
                </div>
                <div>
                  <label htmlFor="rma-siteid" className="block text-sm text-gray-600 mb-1">Site ID / Dest</label>
                  <input 
                    id="rma-siteid" 
                ...
                <div>
                  <label htmlFor="rma-sitename" className="block text-sm text-gray-600 mb-1">Site Name / Destination</label>
                  <div className="relative">
                    <input 
                      id="rma-sitename" 
                      ...
                      list="rma-sitename-options"
                      disabled={!formData.region}
                    />
                    <datalist id="rma-sitename-options">
                ...
                <div>
                  <label htmlFor="rma-sitetype" className="block text-sm text-gray-600 mb-1">Site Type</label>
                  <input 
                    id="rma-sitetype" 
              ...
              <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                <div className="flex items-center mb-2">
                  ...
                  <label className="block text-sm font-medium text-gray-700">Berita Acara File Upload</label>
                </div>
                <p className="text-sm text-gray-600 mb-3">Please upload berita acara file</p>
              ...
              <div className="flex justify-end">
                <button type="submit" disabled={submitting} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-60">
                  {submitting ? 'Saving...' : 'Save Request Material Additional'}
                </button>
              </div>
            </form>
          </div>
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-blue-700">
                  <p>As a Material role user, you can view and filter request material additional data, but cannot add new records. Only RPM users have permission to create new request material additional entries.</p>
                </div>
              </div>
            </div>
          </div>
...
export default RequestMaterialAdditionalPage;
