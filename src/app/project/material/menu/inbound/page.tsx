...
const InboundPage: React.FC = () => {
  ...
  const [filteredData, setFilteredData] = useState<InboundData[]>([]);
  ...
  // Form state for adding a new inbound record
  const [formData, setFormData] = useState<Omit<InboundData, 'id'>>({
    year: '',
    region: '',
    shipper: '',
    driver: '',
  ...
  const numericKeys: Array<keyof Omit<InboundData, 'id'>> = [
  ...
  const handleFormChange = (key: keyof Omit<InboundData, 'id'>, value: string) => {
    ...
    if (key === 'region') {
      setFormData(prev => ({ ...prev, siteIdOrig: '', siteNameOrigin: '', siteType: '' }));
    } else if (key === 'siteNameOrigin') {
      ...
      if (selectedSite) {
        setFormData(prev => ({ 
          ...prev, 
          siteIdOrig: selectedSite.siteId,
  ...
  // Filter functions
  const applyFilters = (data: InboundData[]) => {
    return data.filter(item => {
      ...
      if (filters.siteIdOrig && !item.siteIdOrig.toLowerCase().includes(filters.siteIdOrig.toLowerCase())) return false;
      if (filters.siteNameOrigin && !item.siteNameOrigin.toLowerCase().includes(filters.siteNameOrigin.toLowerCase())) return false;
  ...
  const exportToCSV = () => {
    ...
    try {
      const headers = [
        'Year', 'Region', 'Shipper', 'Driver', 'No POI', 'Site ID Origin', 'Site Name Origin', 'Site Type',
      ...
      const csvContent = [
        ...
        ...filteredData.map(item => [
          item.year, item.region, item.shipper, item.driver, item.noPoi, item.siteIdOrig, item.siteNameOrigin, item.siteType,
      ...
      link.setAttribute('download', `inbound_material_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const requiredFields = [
      ...
      'siteIdOrig', 'siteNameOrigin', 'siteType'
    ];
    
    const emptyFields = requiredFields.filter(field => {
      const value = formData[field as keyof Omit<InboundData, 'id'>];
    ...
    if (emptyFields.length > 0) {
      const fieldNames = emptyFields.map(field => {
        ...
        return field
          ...
          .replace('Id Orig', ' ID Origin')
          .replace('Name Origin', ' Name Origin');
      }).join(', ');
    ...
    try {
      setSubmitting(true);
      const docRef = await addDoc(collection(db, 'material_inbound'), {
        ...formData,
        createdAt: serverTimestamp(),
      });
      // Reset form
      setFormData({
        year: '', region: '', shipper: '', driver: '', noPoi: '', siteIdOrig: '', siteNameOrigin: '', siteType: '',
      ...
      alert('Inbound record saved successfully.');
    } catch (err) {
      console.error('Failed to save inbound record', err);
      alert('Failed to save inbound record.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  useEffect(() => {
    const q = query(collection(db, 'material_inbound'), orderBy('createdAt', 'desc'));
    const unsub = onSnapshot(q, (snapshot) => {
      const data: InboundData[] = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      })) as InboundData[];
      setInboundData(data);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      const sites = snapshot.docs.map(doc => doc.data() as {
        region: string;
        siteId: string;
        siteName: string;
        siteType: string;
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter((item) =>
        item.siteIdOrig.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.siteNameOrigin.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  }, [searchTerm, inboundData, filters]);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading Inbound Data...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-full mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center mb-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Inbound Material</h1>
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            ...
            <h3 className="text-lg font-semibold text-gray-900">Inbound Material Data</h3>
              <div className="text-sm text-gray-600">
                Showing {filteredData.length} of {inboundData.length} records
              </div>
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            {filteredData.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900">No inbound records found</h3>
                <p className="mt-1 text-sm text-gray-500">Get started by adding a new inbound record using the form below.</p>
              </div>
            ) : (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredData.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.siteIdOrig}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.siteNameOrigin}</td>
          ...
          {filteredData.length === 0 && (
            <div className="px-6 py-12 text-center">
              <div className="flex flex-col items-center">
                ...
                <p className="text-gray-500 text-lg font-medium">No inbound data found</p>
        ...
        {userData?.role?.toLowerCase() === 'rpm' && (
          <div className="bg-white rounded-lg shadow-md p-6 mt-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Add Inbound Record</h3>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {/* Text fields */}
                <div>
                  <label htmlFor="f-year" className="block text-sm text-gray-600 mb-1">Year</label>
                  <input id="f-year" value={formData.year} onChange={e=>handleFormChange('year', e.target.value)} className="w-full border rounded px-3 py-2 text-black" />
                </div>
                <div>
                  <label htmlFor="f-region" className="block text-sm text-gray-600 mb-1">Region</label>
                  <div className="relative">
                    <input 
                      id="f-region" 
                ...
                <div>
                  <label htmlFor="f-shipper" className="block text-sm text-gray-600 mb-1">Shipper</label>
                  <input id="f-shipper" value={formData.shipper} onChange={e=>handleFormChange('shipper', e.target.value)} className="w-full border rounded px-3 py-2 text-black" />
                </div>
                <div>
                  <label htmlFor="f-driver" className="block text-sm text-gray-600 mb-1">Driver</label>
                  <input id="f-driver" value={formData.driver} onChange={e=>handleFormChange('driver', e.target.value)} className="w-full border rounded px-3 py-2 text-black" />
                </div>
                <div>
                  <label htmlFor="f-nopoi" className="block text-sm text-gray-600 mb-1">No POI</label>
                  <input id="f-nopoi" value={formData.noPoi} onChange={e=>handleFormChange('noPoi', e.target.value)} className="w-full border rounded px-3 py-2 text-black" />
                </div>
                <div>
                  <label htmlFor="f-siteid" className="block text-sm text-gray-600 mb-1">Site ID / Orig</label>
                  <input 
                    id="f-siteid" 
                    value={formData.siteIdOrig} 
                    onChange={e=>handleFormChange('siteIdOrig', e.target.value)} 
                    className="w-full border rounded px-3 py-2 text-black bg-gray-100" 
                    readOnly
                  />
                </div>
                <div>
                  <label htmlFor="f-sitename" className="block text-sm text-gray-600 mb-1">Site Name / Origin</label>
                  <div className="relative">
                    <input 
                      id="f-sitename" 
                      value={formData.siteNameOrigin} 
                      onChange={e=>handleFormChange('siteNameOrigin', e.target.value)} 
                      className="w-full border rounded px-3 py-2 text-black"
                    ...
                    {formData.siteNameOrigin && formData.siteNameOrigin.length > 0 && (
                ...
                <div>
                  <label htmlFor="f-sitetype" className="block text-sm text-gray-600 mb-1">Site Type</label>
                  <input 
                    id="f-sitetype" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                  <label htmlFor="f-fotopod" className="block text-sm text-gray-600 mb-1">Foto PoD</label>
                  <input id="f-fotopod" value={formData.fotoPod} onChange={e=>handleFormChange('fotoPod', e.target.value)} className="w-full border rounded px-3 py-2 text-black" />
                </div>
                <div>
                  <label htmlFor="f-fotomater" className="block text-sm text-gray-600 mb-1">Foto Mater</label>
                  <input id="f-fotomater" value={formData.fotoMater} onChange={e=>handleFormChange('fotoMater', e.target.value)} className="w-full border rounded px-3 py-2 text-black" />
                </div>
              </div>
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-blue-700">
                  <p>As a Material role user, you can view and filter inbound material data, but cannot add new records. Only RPM users have permission to create new inbound entries.</p>
                </div>
              </div>
            </div>
          </div>
