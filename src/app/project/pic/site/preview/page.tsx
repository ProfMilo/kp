  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const addHeader = async () => {
        ...
        try {
          // Use fetch to get the logo as blob to avoid CORS issues
          ...
          const logoBase64Promise = new Promise<string>((resolve, reject) => {
            logoReader.onload = () => resolve(logoReader.result as string);
            logoReader.onerror = reject;
          });
          logoReader.readAsDataURL(logoBlob);
          const logoData = await logoBase64Promise;
      ...
      for (let sectionIdx = 0; sectionIdx < sectionsToExport.length; sectionIdx++) {
        ...
        for (let i = 0; i < imageFiles.length; i++) {
          ...
          try {
            // Use fetch to get the image as blob to avoid CORS issues with timeout
            ...
            // Add image to PDF
            pdf.addImage(imgData, imageFormat, currentX, currentY, imgWidth, imgHeight);
            ...
            console.error(`Failed to load image ${file.fileName}:`, error);
            ...
            pdf.text('Error: The operation is insecure.', currentX + imgWidth/2, currentY + imgHeight/2 + 16, { align: 'center' });
          }
          imagesOnCurrentPage++;
