        ...
        <tbody>
          {currentItems.map((item, idx) => (
            <tr key={idx} className="border-b border-gray-300 hover:bg-gray-50">
              ...
              <td className="px-2 py-2">{String(item.nik || '-')}</td>
              <td className="px-2 py-2 font-medium">{String(item.name)}</td>
              <td className="px-2 py-2">{String(item.position)}</td>
              <td className="px-2 py-2">{String(item.joinDate)}</td>
              <td className="px-2 py-2">{String(item.phone)}</td>
              <td className="px-2 py-2">
                <a href={`mailto:${item.email}`} className="text-blue-600 hover:underline">
                  {String(item.email)}
                </a>
              </td>
              <td className="px-2 py-2">{String(item.division)}</td>
              <td className="px-2 py-2">{String(item.project)}</td>
              <td className="px-2 py-2">{String(item.region)}</td>
              <td className="px-2 py-2 min-w-[100px]">
                {item.status && String(item.status).toLowerCase() === 'active' ? (
                ...
                ) : item.status && String(item.status).toLowerCase() === 'inactive' ? (
                  ...
                  <span className="bg-gray-500 text-white px-3 py-1 rounded-full text-xs font-bold inline-block text-center whitespace-nowrap">{String(item.status || 'Unknown')}</span>
                )}
              </td>
...
const EmployeeDataPage: React.FC = () => {
    ...
    try {
      ...
      usersSnapshot.forEach((doc) => {
        ...
        if (data.role && data.name) {
          ...
          employees.push({
            id: doc.id,
            nik: safeToString(data.nik || data.employeeId || '-'),
            name: safeToString(data.name),
            position: safeToString(data.position || data.jobTitle || '-'),
            joinDate: convertTimestampToString(data.joinDate || data.hireDate || data.createdAt),
            phone: safeToString(data.phone || data.phoneNumber || '-'),
            email: safeToString(data.email || ''),
            division: safeToString(division),
            project: safeToString(data.project || data.currentProject || '-'),
            region: safeToString(data.region || data.location || '-'),
            status: (() => {
              const statusValue = data.status || data.employeeStatus || 'active';
              if (typeof statusValue === 'string' && ['active', 'inactive', 'resigned'].includes(statusValue.toLowerCase())) {
                return statusValue.toLowerCase() as 'active' | 'inactive' | 'resigned';
              }
  ...
  const filteredEmployees = allEmployees.filter(employee => {
    const matchesDate = !filterDate || String(employee.joinDate).includes(filterDate);
    const matchesDivision = !filterDepartment || String(employee.division).toLowerCase().includes(filterDepartment.toLowerCase());
    const matchesSearch = !searchQuery || 
      ...
      String(employee.position).toLowerCase().includes(searchQuery.toLowerCase()) ||
      String(employee.division).toLowerCase().includes(searchQuery.toLowerCase()) ||
      (employee.nik && String(employee.nik).toLowerCase().includes(searchQuery.toLowerCase())) ||
      String(employee.phone).toLowerCase().includes(searchQuery.toLowerCase()) ||
      String(employee.project).toLowerCase().includes(searchQuery.toLowerCase()) ||
      String(employee.region).toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const groupedData = {
    'Active Employees': filteredEmployees.filter(emp => 
      emp.status && String(emp.status).toLowerCase() === 'active'
    ),
    'Inactive Employees': filteredEmployees.filter(emp => 
      emp.status && String(emp.status).toLowerCase() === 'inactive'
    )
  };

  const totalEmployees = filteredEmployees.filter(emp => emp.status && String(emp.status).toLowerCase() === 'active').length;
          ...
          <div className="flex flex-col items-center lg:items-end gap-3">
            ...
            {searchQuery && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-2 font-medium text-sm text-blue-800 flex flex-row justify-between items-center shadow-sm w-full lg:w-auto">
                ...
                <span className="text-lg text-blue-900 font-semibold ml-4">{filteredEmployees.filter(emp => emp.status && String(emp.status).toLowerCase() === 'active').length}</span>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow">
            <div className="p-4 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">
                Search Results for "{searchQuery}" ({filteredEmployees.filter(emp => emp.status && String(emp.status).toLowerCase() === 'active').length} active results)
              </h3>
            </div>
