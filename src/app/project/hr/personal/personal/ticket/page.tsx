        ...
        <tbody>
          {data.map((item, idx) => (
            <tr key={idx} className="border-b border-gray-300 hover:bg-gray-50">
              <td className="px-3 py-3 text-center">{new Date(item.date).toLocaleDateString()}</td>
              ...
              <td className="px-3 py-3 text-center">{new Date(item.ticketDate).toLocaleDateString()}</td>
...
const TicketPage: React.FC = () => {
  ...
  const handleExportExcel = () => {
    const formattedData = filteredTickets.map(item => ({
      ...
      'Name': item.name,
      'Role': item.role,
      'Ticket Date': new Date(item.ticketDate).toLocaleDateString(),
      ...
      'Rejected Date': item.rejectedDate ? new Date(item.rejectedDate).toLocaleDateString() : '-',
      'Completed By': item.completedBy || '-',
      'Completed Date': item.completedDate ? new Date(item.completedDate).toLocaleDateString() : '-'
    }));
