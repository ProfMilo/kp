        ...
        <tbody>
          {data.map((item, idx) => (
            <tr key={idx} className="border-b border-gray-300 hover:bg-gray-50">
              ...
              <td className="px-4 py-3 text-center">{new Date(item.resignationDate).toLocaleDateString()}</td>
...
const ResignationPage: React.FC = () => {
    ...
    try {
      ...
      resignationsSnapshot.forEach((doc) => {
        ...
        resignations.push({
          ...
          approvedDate: data.approvedDate || data.approvedDate || '',
  ...
  const handleExportExcel = () => {
    const formattedData = filteredResignations.map(item => ({
      ...
      'Resignation Date': new Date(item.resignationDate).toLocaleDateString(),
      'Reason': item.reason,
      'Status': item.status,
      'Approved Date': item.approvedDate ? new Date(item.approvedDate).toLocaleDateString() : '-',
      'Exit Interview': item.exitInterview || '-',
      'Handover Status': item.handoverStatus || '-'
    }));
    ...
    const columnWidths = [
      ...
      { wch: 20 }, // Exit Interview
      { wch: 20 }  // Handover Status
    ];
