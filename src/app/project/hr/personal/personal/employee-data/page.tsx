      ...
      usersSnapshot.forEach((doc) => {
        ...
        if (data.role && data.name) {
          ...
          employees.push({
            ...
            joinDate: data.joinDate || data.hireDate || data.createdAt || '-',
      ...
      <div className="w-full px-2 sm:px-4">
          ...
          <div className="flex flex-col items-center lg:items-end gap-3">
            ...
            {searchQuery && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-2 font-medium text-sm text-blue-800 flex flex-row justify-between items-center shadow-sm w-full lg:w-auto">
                ...
                <span className="text-lg text-blue-900 font-semibold ml-4">{filteredEmployees.length}</span>
              </div>
            )}
          </div>
        </div>
        ...
        {searchQuery && filteredEmployees.length === 0 ? (
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow">
            <div className="p-4 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">
                Search Results for "{searchQuery}" ({filteredEmployees.length} results)
              </h3>
            </div>
          ...
          HR_CATEGORIES.map(cat => (
            ...
            >
              <EmployeeTable data={groupedData[cat as keyof typeof groupedData] || []} />
            </AccordionSection>
          ))
        )}
      </div>
